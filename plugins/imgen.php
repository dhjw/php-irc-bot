...
if ($imgen_config["is_gemini"] && !$imgen_config["ibb_enabled"] && !$imgen_config["github_enabled"]) exit("imgen plugin: using gemini requires ibb_enabled or github_enabled");
if ($imgen_config["ibb_enabled"] && $imgen_config["github_enabled"]) exit("imgen plugin: only enable one of ibb_enabled or github_enabled");
if ($imgen_config["github_enabled"]) $imgen_config["user_repo"] = $imgen_config["github_user"] . '/' . $imgen_config["github_user"] . '.github.io';
if ($imgen_config["convert_png_to_webp"] && !extension_loaded("gd")) exit("[imgen-plugin] The GD extension is required. On Ubuntu or Debian, try sudo apt install php-gd. The best PPA is from https://deb.sury.org\n");
...
function imgen()
{
	...
	if ($imgen_config["is_gemini"]) { // use native endpoint as openai-compatible is buggy or discontinued
		...
		while (true) {
			...
			if (isset($r[0]->error)) return send("PRIVMSG $target :API error: {$r[0]->error->message}\n");
			$has_image = false;
			$blocked = false;
			$finish_reason = false;
			$text = "";
			if (isset($r->error)) return send("PRIVMSG $target :[$r->error->code] " . explode('.', $r->error->message)[0] . "\n");
			foreach ($r as $k => $c) {
				// print_r($c);
			...
			if ($has_image) {
				...
				if ($blocked) return send("PRIVMSG $target :Blocked\n");
				elseif ($finish_reason == "STOP") return send("PRIVMSG $target :Blocked\n");
				else {
					if ($tries >= 3) return send("PRIVMSG $target :Failed to generate image with 3 tries\n");
					else echo "[imgen] Retrying...\n";
				}
			}
		}
		unset($r);
		...
		if (!isset($r->data[0]->url)) return send("PRIVMSG $target :API error\n");
		$url = $r->data[0]->url;
	}
	...
	if ($imgen_config["ibb_enabled"]) {
		...
		if ($imgen_config["ibb_short_urls"]) $r->data->url = substr($r->data->url, 0, strrpos($r->data->url, '/')) . "/i" . substr($r->data->url, strrpos($r->data->url, '.'));
