<?php
// run a gemini query using native endpoint (for more options than the openai-compatible endpoint)
// github pastes same as with llm.php plugin (used for openai-compatible services) - can use same github user/repo
...
// note: $gem_config vars can be modified after plugin inclusion in bot settings file without changing this file

$custom_triggers[] = [$gem_config["trigger"], "function:gem_query", true, $gem_config["trigger"] . " - query Gemini"];
if ($gem_config["github_enabled"]) $gem_config["user_repo"] = $gem_config["github_user"] . '/' . $gem_config["github_user"] . '.github.io';
...
function gem_query()
{
	...
	foreach ($gem_config["services"] as $k => $s) if ($s["trigger"] == $trigger) {
		$gem_config = $s;
		break;
	}
	...
	// image input
	$images = [];
	if (!isset($gem_config["image_cache_mb"])) $gem_config["image_cache_mb"] = 5;
	if (!isset($gem_config["image_cache"])) $gem_config["image_cache"] = [];
	if (preg_match_all("#(https?://[^ ]+)(?:\s|$)#", $args, $m)) {
		$visual_args = $args;
		foreach ($m[1] as $url) {
			$is_image = false;
			// gemini doesn't accept urls, so have to download
			if (isset($gem_config["image_cache"][$url])) $r = $gem_config["image_cache"][$url];
			else {
				...
				while (true) {
					$total = 0;
					foreach ($gem_config["image_cache"] as $k => $v) $total += strlen($k) + strlen($v);
					if ($total / 1024 / 1024 > $gem_config["image_cache_mb"]) array_shift($gem_config["image_cache"]); else break;
				}
			}
	...
	if ((!$args && !$images) || preg_match("#^(?:\.|\.?help$)#", $args)) {
		...
		if (!empty($gem_config["memory_enabled"])) {
			...
			if ($gem_config["memory_max_age"] % 60 == 0) $txt .= ($gem_config["memory_max_age"] / 60) . "m)"; else $txt .= $gem_config["memory_max_age"] . "s)";
		}
	...
	// add past messages
	if ($gem_config["memory_enabled"]) {
		if (!isset($gem_config["memory_items"])) $gem_config["memory_items"] = [];
		// forget expired memories
		if (!empty($gem_config["memory_items"])) foreach (array_reverse($gem_config["memory_items"], true) as $k => $mi) {
			$age = $time - $mi->time;
	...
	$c_obj->parts = [];
	if (!$images || $visual_args) $c_obj->parts[] = (object)["text" => $images ? $visual_args : $args];
	if ($images) $c_obj->parts = array_merge($c_obj->parts, $images);
	$data->contents[] = $c_obj;
