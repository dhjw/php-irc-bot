...
$llm_config = [
	...
	// how to setup github:
	// create a user and a repo, preferably with a short name like "r"
	...
	//   git init; git remote add origin https://user:token@github.com/user/repo.git  # edit to contain your user, token and repo
	...
	//   git checkout -b page; git clone -b page https://github.com/llm10/r.git page  # full example page with images for chatgpt, grok and gemini
...
function llm_query()
{
	...
	if (substr($target, 0, 1) <> "#") return send("PRIVMSG $target :This command only works in $channel\n");

	foreach ($llm_config["services"] as $k => $s) if ($s["trigger"] == $trigger) {
		$service = $s;
		break;
	}
	...
	// image input
	$images = [];
	if (!isset($llm_config["image_cache_mb"])) $llm_config["image_cache_mb"] = 5;
	if (!isset($llm_config["image_cache"])) $llm_config["image_cache"] = [];
	if (preg_match_all("#(https?://[^ ]+)(?:\s|$)#", $args, $m)) {
		...
		foreach ($m[1] as $url) {
			...
			if ($service["name"] == "Gemini" || !preg_match("#^https?://[^ ]+?\.(?:jpg|jpeg|png)#i", $url)) { // gemini doesn't accept urls, so have to download. downloading imgur images may not work, but chatgpt and grok can get them. don't download anything with correct extension. TODO don't download *.webp for chatgpt
				if (isset($llm_config["image_cache"][$url])) $r = $llm_config["image_cache"][$url];
				else {
					$r = curlget([CURLOPT_URL => $url]);
					if (empty($r)) {
						if (!empty($curl_error) && strpos($curl_error, "Operation timed out") !== false) return send("PRIVMSG $target :Timeout getting image\n");
						return send("PRIVMSG $target :Failed to get image\n");
					}
					$llm_config["image_cache"][$url] = $r;
					while (true) {
						$total = 0;
						foreach ($llm_config["image_cache"] as $k => $v) $total += strlen($k) + strlen($v);
						if ($total / 1024 / 1024 > $llm_config["image_cache_mb"]) array_shift($llm_config["image_cache"]); else break;
					}
				}
				...
				if (preg_match("#image/(?:jpeg|png|webp|avif|gif)#", $mime)) {
					...
					if (preg_match("#image/(?:webp|avif|gif)#", $mime)) { // convert to png and use data-uri // TODO don't convert webp for chatgpt
						...
						if (!$im) return send("PRIVMSG $target :Error converting $mime image\n");
						ob_start();
						imagepng($im);
						$im = ob_get_clean();
						$image_url = "data:image/png;base64," . base64_encode($im);
					} elseif ($service["name"] == "Gemini") $image_url = "data:$mime;base64," . base64_encode($r);
					else $image_url = $url;
				}
			} else {
	...
	if ((!$args && !$images) || preg_match("#^(?:\.|\.?help$)#", $args)) {
		...
		if (!empty($llm_config["memory_enabled"])) {
			...
			if ($llm_config["memory_max_age"] % 60 == 0) $txt .= ($llm_config["memory_max_age"] / 60) . "m)"; else $txt .= $llm_config["memory_max_age"] . "s)";
		}
	...
	if ($images) $msg_obj->content = array_merge($msg_obj->content, $images);
	$data->messages[] = $msg_obj;
	if ($images) $data->model = $service["vision_model"];
	...
	$r = @json_decode($r);
	if (empty($r)) {
		if (!empty($curl_error) && strpos($curl_error, "Operation timed out") !== false) return send("PRIVMSG $target :" . $service["name"] . " API error: timeout\n");
	...
	if (($llm_config["github_enabled"] && count($out_lines) >= $llm_config["github_min_lines"]) || $github_force) {
		...
		if ($r->status == 404) {
			...
			$r = curlget([
				CURLOPT_URL => "https://api.github.com/repos/" . $llm_config["github_user"] . "/" . $llm_config["github_repo"] . "/actions/variables",
				CURLOPT_CUSTOMREQUEST => "POST",
				CURLOPT_HTTPHEADER => ["Accept: application/vnd.github+json", "Authorization: Bearer " . $llm_config["github_token"], "X-GitHub-Api-Version: 2022-11-28"],
				CURLOPT_POSTFIELDS => json_encode($data)
			], ["no_curl_impersonate" => 1]);
			...
			$r = @json_decode($r);
			if (!empty($curl_error) && strpos($curl_error, "Operation timed out") !== false) return send("PRIVMSG $target :GitHub timeout\n");
			if ($curl_info["RESPONSE_CODE"] !== 201) return send("PRIVMSG $target :GitHub error creating index var\n");
			$github_index = 1;
		...
		$r = curlget([
			CURLOPT_URL => "https://api.github.com/repos/" . $llm_config["github_user"] . "/" . $llm_config["github_repo"] . "/contents/$github_path",
		...
		$r = @json_decode($r);
		if (!empty($curl_error) && strpos($curl_error, "Operation timed out") !== false) return send("PRIVMSG $target :GitHub timeout\n");
		if ($curl_info["RESPONSE_CODE"] !== 201) return send("PRIVMSG $target :GitHub error\n");
